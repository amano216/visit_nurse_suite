        ...
        return `
            ...
            <div class="form-group">
                ...
                <select id="woundPocket">
                    <option value="0">なし（+0）</option>
                    <option value="1">面積 &lt;4 cm²（+6）</option>
            ...
            <div class="citation" style="font-size:0.9em; color:#555; margin-top:12px;">
                <strong>【出典】</strong> 一般社団法人 日本褥瘡学会. 「改定DESIGN-R®︎2020」（参照2025-4-28）／
                一般社団法人 日本褥瘡学会. 「褥瘡状態評価スケール 改定DESIGN-R®︎2020 コンセンサス・ドキュメント」. 照林社, 2020, 30p
            </div>
        `;
    }
...
class WoundCalculator {
    calculate() {
        ...
        // Size（大きさ）: 面積=長径×短径(cm²)
        // 0=0, <4=3, 4~<16=6, 16~<36=8, 36~<64=9, 64~<100=12, ≧100=15
        const lengthCm = parseFloat(document.getElementById('woundSizeLong')?.value) || 0;
        const shortCm = parseFloat(document.getElementById('woundSizeShort')?.value) || 0;
        const area = lengthCm * shortCm;
        let sizeScore = 0;
        if (area === 0) sizeScore = 0;
        else if (area < 4) sizeScore = 3;
        else if (area < 16) sizeScore = 6;
        else if (area < 36) sizeScore = 8;
        else if (area < 64) sizeScore = 9;
        else if (area < 100) sizeScore = 12;
        else sizeScore = 15;
        // Inflammation/Infection（炎症/感染）
        // 0:なし=0, 1:炎症徴候あり=1, 2:臨界的定着疑い=3, 3:局所感染=3, 4:全身性影響=9
        ...
        // Granulation（肉芽組織）:
        // 0:治癒/浅い/DTI疑い=0, 1:≧90%=1, 2:50~<90%=3, 3:10~<50%=4, 4:<10%=5, 5:なし=6
        ...
        // Necrosis（壊死組織）: 0:なし=0, 1:柔らかい壊死=3, 2:硬く厚い壊死=6
        ...
        this.displayResult(totalScore, {
            depth, exudate, area, lengthCm, shortCm, sizeScore, inflammation, granulation, necrosis, pocket
        });
    }
    ...
    const pocketTexts = ['なし（+0）', '<4（+6）', '4~<16（+9）', '16~<36（+12）', '≧36（+24）'];
        ...
        resultDiv.innerHTML = `
            ...
            <div class="result-item">
                <strong>大きさ:</strong> 長径 ${values.lengthCm ?? 0} cm × 短径 ${values.shortCm ?? 0} cm = 面積 ${values.area ?? 0} cm² (${values.sizeScore}点)
            </div>
            <div class="result-item">
    ...
    reset() {
        ...
        const sizeLongEl = document.getElementById('woundSizeLong');
        const sizeShortEl = document.getElementById('woundSizeShort');
        if (sizeLongEl) sizeLongEl.value = '';
        if (sizeShortEl) sizeShortEl.value = '';
        const resultDiv = document.getElementById('woundResult');
        if (resultDiv) resultDiv.style.display = 'none';
    }
}
